...
void PicoGFX_SSD1322::display(void) {
  ...
  uint8_t column_start = col_offset + (byte_start / 2);  // Map byte index to column
  uint8_t column_end = col_offset + (byte_end / 2);      // Map byte index to column
  transfer_byte_start = (column_start - col_offset) * 2; // Starting byte offset
  ...
  // Sync: copy displayed content into new draw buffer so partial
  // updates don't expose stale data from a previous frame
  uint16_t buf_size = WIDTH * HEIGHT / 2;
  memcpy(drawBuffer, displayBuffer, buf_size);
